...
def X_values2():
    ...
    X_values2.val = [box3.get(idx) for idx in box3.curselection()]
    
    for i in range(len(list(X_values2.val))):
        box4.insert(i, X_values2.val[i])
        box3.selection_clear(i, END)
    print("Predictors are:"+ str(X_values2.val))
...
def Kmeans():
    ...
    X = df.iloc[:, list(x_vars)].values
    X = encodingc(X_values2.val, X)
    kmeans = KMeans(n_clusters=k)
    kmeans.fit(X)
    pcaNumber = int(len(x_vars))
...
def hca():
    ...
    try:
        ...
        if X.shape[1] == 1:
            ...
            plt.title(f'Silhouette Score: {silhouette_avg:.2f}\nClusters of customers (1D)')
            plt.xlabel('Feature')
            plt.yticks([])
        elif X.shape[1] >= 2:
...
button_showScatterPlot.grid(row=2, column=0, padx=50, pady=0)
...
button_showHeatMap.grid(row=1, column=1, padx=0, pady=30)
...
button2t1.grid(row=15, column=0, padx=50, pady=94)

button2t3 = tk.Button(page2, text="->",font=sty6, command=lambda: page3.tkraise())
button2t3.grid(row=15, column=2, padx=115, pady=94)
...
OPTIONS = ["Select model","linear","poly","rbf"]
kernelVar = StringVar()
...
LabelFrame14.pack(anchor='w',ipady=5,padx=110)
...
con = tk.Frame(page4)
con.pack(anchor='w', pady=12, padx=20, ipady=0)

#--------------------------Kmeans section----------------------#

kmeans_frame = LabelFrame(con, text="KMeans")
kmeans_frame.pack(side='left', padx=5, pady=0)

dk_button = tk.Button(kmeans_frame, text="Determine K", command=deter_k)
dk_button.grid(row=0, column=0, padx=30, pady=5)

km_label = tk.Label(kmeans_frame, text="Enter the value of K:")
km_label.grid(row=1, column=0, padx=30, pady=5)

km_entry = tk.Entry(kmeans_frame)
km_entry.grid(row=2, column=0, padx=30, pady=1)
...
drop.grid(row=3, column=0, columnspan=2, padx=30, pady=5)

km_button = tk.Button(kmeans_frame, text="KMeans", command=Kmeans)
km_button.grid(row=4, column=0, columnspan=2, padx=30, pady=5)

#------------------------HCA Section----------------------#

hca_frame = LabelFrame(con, text="HCA")
hca_frame.pack(side='left', padx=30, pady=0)

dchca_button = tk.Button(hca_frame, text="Determine clusters", command=deter_hca)
dchca_button.grid(row=1, column=0, padx=30, pady=5)

hca_label = tk.Label(hca_frame, text="clusters:")
hca_label.grid(row=2, column=0, padx=30, pady=5)

hca_entry = tk.Entry(hca_frame)
hca_entry.grid(row=3, column=0, padx=30, pady=1)

options = ["euclidean", "manhattan", "jaccard", "cosine", "minkowski"]
clicked = StringVar()
clicked.set("euclidean")
drop = OptionMenu(hca_frame, clicked, *options)
drop.grid(row=4, column=0, columnspan=2, padx=30, pady=5)
...
dropLinkage.grid(row=5, column=0, columnspan=2, padx=30, pady=5)

hca_button = tk.Button(hca_frame, text="HCA", command=hca)
hca_button.grid(row=6, column=0, columnspan=2, padx=30, pady=5)

#-------------------DB-Scan Section---------------------#
dbscan_frame = LabelFrame(con, text="DB-Scan")
dbscan_frame.pack(side='left', padx=10, pady=0)

db_label = tk.Label(dbscan_frame, text="min point:")
db_label.grid(row=0, column=0, padx=5, pady=1)

db_entry = tk.Entry(dbscan_frame)
db_entry.grid(row=0, column=1, padx=5, pady=1)

eps_label = tk.Label(dbscan_frame, text="eps:")
...
dropcmap.grid(row=2, column=0, columnspan=2, padx=5, pady=5)

db_button = tk.Button(dbscan_frame, text="DB-Scan", command=dbscan)
db_button.grid(row=3, column=0, columnspan=2, padx=5, pady=5)

containerback2 = tk.Frame(page4)
containerback2.pack(anchor='w', pady=0, padx=20, ipadx=165)

button4t3 = tk.Button(containerback2, text="<-",font=sty6, command=lambda: page3.tkraise())
button4t3.pack(side='left',anchor='w',padx=97,pady=0)

button4t5 = tk.Button(containerback2, text="->Coming soon",font=sty6)
button4t5.pack(side='right',anchor='n',pady=0)
